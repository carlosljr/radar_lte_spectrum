    ...
    m_cellIdCounter (0)
{
  ...
  //Inserção --> Carlos
  m_radarFactory.SetTypeId (radar::GetTypeId ());
  m_dsaFactory.SetTypeId (DsaMechanisms::GetTypeId ());

  //Inserção --> Carlos: Instanciando objeto para o cálculo da interferência agregada.
  m_agg = CreateObject<AggregateInterference> ();
  m_propagationDelayFactory.SetTypeId (ConstantSpeedPropagationDelayModel::GetTypeId ()); 

  //Inserção --> Carlos: Instanciando objeto para o mecanismo DSA a ser utilizado.
  //m_dsa = m_dsaFactory.Create<DsaMechanisms> (); 
...
LteHelper::DoInitialize (void)
{
  ...
  m_dsa = m_dsaFactory.Create<DsaMechanisms> (); 
  m_propagationDelay = m_propagationDelayFactory.Create<PropagationDelayModel> ();
...
LteHelper::InstallEnbDevice (NodeContainer c)
{
  ...
  //m_dsa = CreateObject<DsaMechanisms> ();
...
//Inserção: Carlos -> Método para instanciar posição do radar.

void
LteHelper::SetRadarMobility (NodeContainer radar) {

	for (NodeContainer::Iterator i = radar.Begin (); i != radar.End (); ++i) {
		Ptr<Node> n = *i;
		m_radarMobility = n->GetObject<MobilityModel> ();
		//std::cout <<"Pegou mobilidade radar: " << m_radarMobility << std::endl;

	}
	m_radarSpectrum = true;
...
LteHelper::InstallSingleEnbDevice (Ptr<Node> n)
{
  ...
  //Inserção -> Carlos: Associando objeto instanciado ao objeto radar de cada dispositivo para cálculo da interferência agregada.

  dlPhy->SetAggregateInterference (m_agg);

  if (m_radarSpectrum) {

  	dlPhy->GetRadar ()->SetMobility (m_radarMobility);
  	ulPhy->GetRadar ()->SetMobility (m_radarMobility);
	dlPhy->SetPropagationDelay (m_propagationDelay);
	ulPhy->SetPropagationDelay (m_propagationDelay);
  }
  
  //Inserção -> Carlos: Associando objeto instanciado ao objeto phy de cada dispositivo para o mecanismo DSA a ser usado.

  phy->SetDsaMechanisms (m_dsa);
...
LteHelper::InstallSingleUeDevice (Ptr<Node> n)
{
  ...
  ulPhy->SetAggregateInterference (m_agg);

  //Inserção -> Carlos: Setar mobilidade do radar.
...
//Insercao --> Carlos: Inserindo m�todo que atribui tempo a partir do qual a eNB ira utilizar a banda do radar.

void
LteHelper::SetStartTime (Time t, Ptr<LteEnbNetDevice> dev) {

	dev->GetPhy ()->SetStartTime (t);
}
