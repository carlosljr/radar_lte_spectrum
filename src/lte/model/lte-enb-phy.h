...
//Inserção: Carlos --> Adicionando a biblioteca radar.
...
public:
  ...
  */
  ...
  //Inserção: Carlos --> Declarando método que seta ponteiro para o objeto DsaMechanisms.

  void SetDsaMechanisms (Ptr<DsaMechanisms> dsa);

  void DecrementInterf (double int_SU);
...
//Inserção Carlos: Declarando Ptr do tipo radar.

  Ptr<radar> m_radar;

  Ptr<DsaMechanisms> m_dsa;

//Booleano que verifica se é a primeira transmissão.
  bool m_isFirst;

  uint16_t m_dlEarfcnOld;
  uint16_t m_ulEarfcnOld;
  bool m_firstDSA;
  bool m_sensing;
  bool m_config;
