...
LteInterference::DoDispose ()
{
  ...
  m_allSignals = 0;
  m_noise = 0;
  //Inserção: Carlos --> Instanciando ponteiro do tipo radar.
  m_radar = 0;
  m_int_radar = 0;
  Object::DoDispose ();
} 
...
LteInterference::AddSignal (Ptr<const SpectrumValue> spd, const Time duration)
{
  ...
  if (m_allSignals->GetSpectrumModelUid () == spd->GetSpectrumModelUid ()) {
	//std::cout << "It has the same spectrum model!" << std::endl;
  	DoAddSignal (spd);
	...
	//std::cout << "Interference from the old channel! Ignoring..." << std::endl;
  }
...
{ 
  ...
  ConditionallyEvaluateChunk ();
   //std::cout << "Volta para DoAddSignal" << std::endl;
   (*m_allSignals) += (*spd);
  ...
  if (m_receiving && (Now () > m_lastChangeTime))
    {
      ...
      //Inserção: Carlos --> Adicionando interferência do radar.

      bool isRadar = m_radar->GetSpectrumRadar ();

      if (isRadar) {

      	(*m_int_radar) = m_radar->get_interference (Now (), m_lastChangeTime);

      	NS_LOG_LOGIC (this << " radar interference = " << *m_int_radar);

      	interf =  (*m_allSignals) - (*m_rxSignal) + (*m_noise) + (*m_int_radar);

      	NS_LOG_LOGIC (this << " total interference = " << interf);

     } else {

	interf =  (*m_allSignals) - (*m_rxSignal) + (*m_noise);

	NS_LOG_LOGIC (this << " total interference without radar = " << interf);
      ...
      //Inserção: Carlos --> Testando ganho e posição angular de nós e radar. Pegando valor da interferência do radar.

      //double ref = m_radar->get_reference(m_lastChangeTime);

      //double G = m_radar->get_gain(ref);

      //ref = m_radar->get_reference(Now ());

      //G = m_radar->get_gain(ref);

      //double interf_radar;

      //interf_radar = m_radar->get_interference (Now (), m_lastChangeTime);
...
LteInterference::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
  ...
  //Inserção Carlos: Setando ponteiro do tipo SpectrumValue para a interferência do radar.
  m_int_radar = Create<SpectrumValue> (noisePsd->GetSpectrumModel ());
  if (m_receiving == true)
    {
      // abort rx
      m_receiving = false;
...
//Inserção: Carlos --> Definindo método que seta o atributo m_radar.


void
LteInterference::SetRadar (Ptr<radar> r) {
