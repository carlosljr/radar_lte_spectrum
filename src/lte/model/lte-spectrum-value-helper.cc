...
LteSpectrumValueHelper::GetCarrierFrequency (uint16_t earfcn)
{
  ...
  if (earfcn < 7000)
	...
	//std::cout << "DL Carrier Frequency: " << (5.6e9 + (2*(1.5e6))) << std::endl;

	return (5.6e9 + (2*(1.5e6))); //HARDCODE: Levando em conta 15 RB's. Modificar para: fc_radar + 2*(B_RB/2.0). Pegar frequência do radar. Elaborar um método... Pegar banda do RB.
   } else if (earfcn == 39651) {

	//std::cout << "UL Carrier Frequency: " << (5.6e9 - (2*(1.5e6))) << std::endl;
...
LteSpectrumValueHelper::GetSpectrumModel (uint16_t earfcn, uint8_t txBandwidthConfiguration)
{
  ...
  //std::cout << "Earfcn for Spectrum model Uid " << ret->GetUid () << " : " << earfcn << std::endl;
  return ret;
}
...
LteSpectrumValueHelper::CreateTxPowerSpectralDensity (uint16_t earfcn, uint8_t txBandwidthConfiguration, double powerTx, std::vector <int> activeRbs)
{
  ...
  //std::cout <<"Power Density: " << txPowerDensity << std::endl;
