...
//Inserção: Carlos: Chamando método para associar instância do objeto radar da classe Spectrum no objeto radar do EnbPhy. Usado no sentido de transmissão da eNB para o cálculo de interferência no radar.
  m_radar = m_uplinkSpectrumPhy->GetRadar ();
  ...
  if (m_ulConfigured)
    {
	  ...
	  if (m_radar->GetSpectrumRadar ()) {
		...
		//std::cout << "UE transmit data traffic in radar band." << std::endl;
	  }
      ...
      else
        {
	      ...
	      if (m_radar->GetSpectrumRadar ()) {
              	double ptx = GetTxPower ();
  	      	m_radar->SetSUTxPower (ptx);
		//std::cout << "UE transmit ctrl msgs in radar band." << std::endl;
	      }
...
LteUePhy::SendSrs ()
{
  ...
  if (m_radar->GetSpectrumRadar ()) {
 	...
 	double ptx = GetTxPower ();
  	m_radar->SetSUTxPower (ptx);
	//std::cout << "Transmit Srs on radar band." << std::endl;
  }
