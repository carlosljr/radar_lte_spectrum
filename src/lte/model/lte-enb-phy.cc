    ...
    m_interferenceSampleCounter (0)
{
  ...
  //Inserção: Carlos: Chamando método para associar instância do objeto radar da classe Spectrum no objeto radar do EnbPhy. Usado no sentido de transmissão da eNB para o cálculo de interferência no radar.
  m_radar = m_downlinkSpectrumPhy->GetRadar ();

  //Seta booleano que verifica se é a primeira transmissão.
...
LteEnbPhy::SendControlChannels (std::list<Ptr<LteControlMessage> > ctrlMsgList)
{
  ...
  //Inserir condição para verificar se é a primeira transmissão. Caso seja, não verifica uso da banda do radar.

  bool dsa_radar, opp;
  Ptr<SpectrumValue> noisePsd;
  Ptr<LteUePhy> uePhy;
  Ptr<LteSpectrumPhy> ueSpecPhy;
  uint16_t cellId;

if ((GetTxPower () <= 20) && (m_dsa->GetDsaMechanism () != "NoRadar")) {

  //std::cout << "It has permission to use radar bands." << std::endl;

  if (m_isFirst && (m_dsa->GetDsaMechanism () != "None")) {

	m_isFirst = false;
        dsa_radar = false;
	m_dlEarfcnOld = m_dlEarfcn;
	m_ulEarfcnOld = m_ulEarfcn;	
	//std::cout << "It is the first transmission. It will use regular LTE band." << std::endl;
		  //<< "DlEarfcn: " << m_dlEarfcnOld << std::endl
		  //<< "UlEarfcn: " << m_ulEarfcnOld << std::endl;

	//m_radar->SetSpectrumRadar (dsa_radar);

  } else {

	double Pthr;

	if (!m_ueAttached.empty()) {

  		//Inserir método que verifica se a banda de radar pode ser usada de forma oportunista.

		//Pegar Ithr e Iagg atual.

		//m_dsa->SetAggInt (m_uplinkSpectrumPhy->GetAgg ()->GetAggInt ());

		//m_dsa->SetIthr (m_radar->GetIthr ());

        	double Ithr = m_radar->GetIthr ();
		double Ptx = GetTxPower ();
		double Ptr = m_radar->GetTxPower ();
		Pthr = Ptr - Ptx + Ithr;

		m_radar->VerPositionRx (m_downlinkSpectrumPhy->GetMobility ()->GetPosition ());

		double Prradar = m_radar->get_interference (Now (), Now ());

		//std::cout << "Pthr: " << Pthr << std::endl
		  //<< "Radar rec Power: " << Prradar << std::endl; 

		if (m_dsa->GetDsaMechanism () == "DFS") {

			//std::cout << "Using DFS..." << std::endl;

			if (m_sensing) {

				opp = m_dsa->DFSGetDsaCondition (Prradar);

				if (opp) {

					uint32_t devices = m_downlinkSpectrumPhy->GetChannel ()->GetNDevices ();
	
					//std::cout << "Number of devices on downlink channel: " << devices << std::endl;

		         		for (uint32_t i=1; i <= devices; i++) {

						//Verificar quais Phy são UEs.

						//std::string name = m_downlinkSpectrumPhy->GetChannel ()->GetDevice (i)->GetTypeId ()->GetName ();

						//if (name == "ns3::LteUeNetDevice") {}

						//Filtrar os Phy por UEs associados a eNB.

						cellId = m_downlinkSpectrumPhy->GetChannel ()->GetObject<MultiModelSpectrumChannel> ()->GetLteDevice (i)->GetObject<LteUeNetDevice> ()->GetRrc ()->GetCellId ();

						//std::cout << "cellId " << cellId << std::endl;

						if (m_cellId == cellId) {

							//std::cout << "UE attached to that eNB! " << std::endl;

							ueSpecPhy = m_downlinkSpectrumPhy->GetChannel ()->GetObject<MultiModelSpectrumChannel> ()->GetLteDevice (i)->GetObject<LteUeNetDevice> ()->GetPhy ()->GetDlSpectrumPhy ();
							m_radar->VerPositionRx (ueSpecPhy->GetMobility ()->GetPosition ());
							opp = m_dsa->DFSGetDsaCondition (m_radar->get_interference (Now (), Now ()));
							if (!opp) {
		...
		} else if (m_dsa->GetDsaMechanism () == "DFST") {

			//std::cout << "Using DFST..." << std::endl;
		...
		} else if (m_dsa->GetDsaMechanism () == "MainBeam") {

			double Gnow = m_radar->get_gain (m_radar->GetNodeAngle (), m_radar->get_reference (Now()));
			double Gmax = m_radar->GetGmax ();
			opp = m_dsa->MainBeamGetDsaCondition (Gmax, Gnow, Prradar, Pthr);
			if (opp) {
				...
				//std::cout << "Number of devices on downlink channel: " << devices << std::endl;
					...
					if (m_cellId == cellId) {
						...
						ueSpecPhy = m_downlinkSpectrumPhy->GetChannel ()->GetObject<MultiModelSpectrumChannel> ()->GetLteDevice (i)->GetObject<LteUeNetDevice> ()->GetPhy ()->GetDlSpectrumPhy ();
						m_radar->VerPositionRx (ueSpecPhy->GetMobility ()->GetPosition ());
						opp = m_dsa->MainBeamGetDsaCondition (Gmax, Gnow, m_radar->get_interference (Now (), Now ()), Pthr);
						if (!opp) {
		...
		} else if (m_dsa->GetDsaMechanism () == "Coop") {

		//std::cout << "Cooperative Mode" << std::endl;

			m_radar->SetPositionTx (m_downlinkSpectrumPhy->GetMobility ()->GetPosition ());

			m_radar->SetSUTxPower (GetTxPower ());

			double SU_interf =  m_radar->get_interference (Now (), Now ());

			m_dsa->SetSUInterference (SU_interf);

			double Gmax = m_radar->GetGmax ();

			m_dsa->SetGmax (Gmax);

			m_dsa->SetGain (m_radar->get_reference (Now ()));

			Time max_TXEndTime = Simulator::Now () + MilliSeconds (2);

			double Gfut = m_radar->get_reference (max_TXEndTime);

			if (Gfut == Gmax) {

				m_dsa->SetNextBoolean (true);

				m_dsa->SetMaxInterference (m_radar->get_interference (max_TXEndTime, max_TXEndTime));

			} else {

				m_dsa->SetNextBoolean (false);

			}


			opp = m_dsa->CoopGetDsaCondition (Ptr, Ptx, m_radar->GetAggregateInterference ()->GetAggInt (), Prradar, Ithr);

		} else if (m_dsa->GetDsaMechanism () == "None") {

		//std::cout << "No DSA Mechanism..." << std::endl;
		...
		//std::cout << "No DSA Mechanism with lte first..." << std::endl;
  	...
  	//dsa_radar = m_dsa->GetDsaCondition ();
	...
	if (opp) {
		...
		//m_radar->SetSpectrumRadar (true);
  		//m_uplinkSpectrumPhy->GetRadar ()->SetSpectrumRadar (true);
  
		//std::cout << "Transmit in radar band." << Pthr << std::endl;

		if (!m_firstDSA) {

			//dsa_radar = false;
			dsa_radar = true;
			//std::cout << "Already using radar band. " << std::endl;

		} else {

			dsa_radar = true;
			//m_firstDSA = false;
			//std::cout << "Not using radar band. Configure transmission parameters. " << std::endl;
		...
		//std::cout << "Do not use radar band!" << Pthr << std::endl;
		...
		m_radar->SetSpectrumRadar (dsa_radar);
  		m_uplinkSpectrumPhy->GetRadar ()->SetSpectrumRadar (dsa_radar);
  
		if (m_dlEarfcn == m_radar->GetDlEarfcn ()) {

			//std::cout << "Reconfiguring transmission parameters to LTE band. Its on the temporal zone." << Pthr << std::endl;

			 m_netDevice->GetObject<LteEnbNetDevice> ()->SetDlEarfcn (m_dlEarfcnOld);
			 m_netDevice->GetObject<LteEnbNetDevice> ()->SetUlEarfcn (m_ulEarfcnOld);
			 DoSetEarfcn (m_ulEarfcnOld, m_dlEarfcnOld);
			 noisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_ulEarfcn, m_ulBandwidth, m_noiseFigure);
			 m_uplinkSpectrumPhy->SetNoisePowerSpectralDensity (noisePsd);
			 m_uplinkSpectrumPhy->GetChannel ()->AddRx (m_uplinkSpectrumPhy);
			 uint32_t devices = m_downlinkSpectrumPhy->GetChannel ()->GetNDevices ();
			...
			//std::cout << "Number of devices on downlink channel: " << devices << std::endl;
				...
				if (m_cellId == cellId) {
					...
					uePhy = m_downlinkSpectrumPhy->GetChannel ()->GetObject<MultiModelSpectrumChannel> ()->GetLteDevice (i)->GetObject<LteUeNetDevice> ()->GetPhy ();
					uePhy->DoSynchronizeWithEnb (m_cellId, m_dlEarfcn);
					uePhy->DoConfigureUplink (m_ulEarfcn, m_ulBandwidth);
					uePhy->DoSetDlBandwidth (m_dlBandwidth);
					uePhy->GetDlSpectrumPhy ()->GetRadar ()->SetSpectrumRadar (dsa_radar);
					uePhy->GetUlSpectrumPhy ()->GetRadar ()->SetSpectrumRadar (dsa_radar);
 ...
 if ((dsa_radar) || (m_dsa->GetDsaMechanism () == "None")) {

   if (m_dsa->GetDsaMechanism () != "None") {
    	m_firstDSA = false;
   }
   if (!m_config) {
    m_config = true;
    ...
    //std::cout << "Configuring transmission parameters to radar band." << std::endl;


     //Configurar os seguintes parâmetros:
	//TX eNB:

	m_netDevice->GetObject<LteEnbNetDevice> ()->SetDlEarfcn (m_radar->GetDlEarfcn ());
	m_netDevice->GetObject<LteEnbNetDevice> ()->SetUlEarfcn (m_radar->GetUlEarfcn ());
	uint16_t ulEarfcn = m_radar->GetUlEarfcn ();
	uint16_t dlEarfcn = m_radar->GetDlEarfcn ();

	//std::cout << "Radar DL-EARFCN: " << dlEarfcn << std::endl
		  //<< "Radar UL-EARFCN: " << ulEarfcn << std::endl;
	DoSetEarfcn (ulEarfcn, dlEarfcn);
        //Realizar procedimento de criação dos subcanais de downlink (Código já existente logo abaixo);
        //RX eNB: Necessário, pois indica aos UEs a transmissão pelo uplink através da banda de radar. Logo, parametros do uplink serão alterados.
	noisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (m_ulEarfcn, m_ulBandwidth, m_noiseFigure);
        m_uplinkSpectrumPhy->SetNoisePowerSpectralDensity (noisePsd);
	//Informar a classe MultiModelChannel o novo canal de RX para SpectrumPhy de uplink.
	m_uplinkSpectrumPhy->GetChannel ()->AddRx (m_uplinkSpectrumPhy);
        //TX UEs associados a essa eNB:

	//Encontrar UEs associados ao eNB.

	//Verificar a quantidade de dispositivos associados a classe MultiModelChannel.
	...
	for (uint32_t i=1; i <= devices; i++) {
		...
		//if (name == "ns3::LteUeNetDevice") {}
			...
			if (m_cellId == cellId) {
				...
				//std::cout << "UE attached to that eNB. Switching channel. " << std::endl;
				...
				uePhy->GetDlSpectrumPhy ()->GetRadar ()->SetSpectrumRadar (dsa_radar);
				uePhy->GetUlSpectrumPhy ()->GetRadar ()->SetSpectrumRadar (dsa_radar);
				//uePhy->SetSubChannelsForTransmission (uePhy->GetSubChannelsForTransmission);

				//RX UEs associados a essa eNB:
	...
	//Inserção: Carlos --> Retorna potência de transmissão do dispositivo LTE que irá transmitir no momento.
  	double ptx = GetTxPower ();
  	m_radar->SetSUTxPower (ptx);

  }

} else {
	//std::cout << "It doesn't have permission to use radar band." << std::endl;
} 
...
//Inserção: Carlos --> Retorna potência de transmissão do dispositivo LTE que irá transmitir no momento.
  if (m_radar->GetSpectrumRadar ()) {
  	double ptx = GetTxPower ();
  	m_radar->SetSUTxPower (ptx);
	//std::cout << "Transmit eNB data traffic on the radar band." << std::endl;
  }
...
//Inserção: Carlos --> Método que seta ponteiro DsaMechanisms.

void
LteEnbPhy::SetDsaMechanisms (Ptr<DsaMechanisms> dsa) {
