  ...
  m_layersNum (1)
{
  ...
  //Inserção Carlos --> Instanciando objeto radar.
  m_radar = CreateObject<radar> ();
  //Inserção Carlos --> Instanciando objeto radar nos objetos Interference.
  m_interferenceData->SetRadar(m_radar);
  m_interferenceCtrl->SetRadar(m_radar);
...
void LteSpectrumPhy::DoDispose ()
{
  ...
  //Inserção Carlos --> Zerando ponteiro.
  m_radar = 0;
  ...
  switch (m_state)
  {
    ...
    case IDLE:
    {
      ...
      */
      ...
      bool isusingRadar = m_radar->GetSpectrumRadar ();
	...
	if (m_propagationDelay) {
		...
		delay = m_propagationDelay->GetDelay (GetMobility (), m_radar->GetMobility ());
	...
	//Inserção: Carlos --> Agendando inserção da interferência no radar.
	Simulator::Schedule (delay, &LteSpectrumPhy::AddSUInterference, this, duration, delay);
      	

	//m_radar -> SetTxDuration (duration);

      	//m_radar->SetPositionTx (m_mobility->GetPosition()); 
      	//Time t_now = Simulator::Now ();
      	//m_radar->CalculateSUInterference (t_now);
	Simulator::Schedule (duration, &LteSpectrumPhy::EndRadarInterf, this);
...
void
LteSpectrumPhy::AddSUInterference (Time dur, Time del) {

	//std::cout << "Adicionando interferência no radar..." << std::endl;

      	//Inserção: Carlos --> Pega a duração da transmissão para cálculo de interferência no radar.
	Time duration = dur - del;
	
      	//Inserção: Carlos --> Pegando valor de tempo atual e calculando interferência resultante da transmissão.
  ...
  switch (m_state)
  {
    ...
    case IDLE:
    {
      ...
      */
      ...
      //SpectrumModelUid_t uid = m_txPsd->GetSpectrumModelUid ();

      //NS_LOG_LOGIC (this << " uid number: " << uid);
	...
	//std::cout << "Entrou na condicao. Usando banda de radar." << std::endl;

	//Time delay = m_channel->GetObject<MultiModelSpectrumChannel> ()->GetPropagationDelay ()->GetDelay (GetMobility (), m_radar->GetMobility ());


	//std::cout << "Configurou delay." << std::endl;

	//Inserção: Carlos --> Agendando inserção da interferência no radar.
	Simulator::Schedule (delay, &LteSpectrumPhy::AddSUInterference, this, DL_CTRL_DURATION, delay);

	//std::cout << "Agendou delay..." << std::endl;
      	
      	//m_radar -> SetTxDuration (DL_CTRL_DURATION);

      	//m_radar->SetPositionTx (m_mobility->GetPosition()); 
      	//Time t_now = Simulator::Now ();
      	//m_radar->CalculateSUInterference (t_now);
	Simulator::Schedule (DL_CTRL_DURATION, &LteSpectrumPhy::EndRadarInterf, this);
...
LteSpectrumPhy::StartTxUlSrsFrame ()
{
  ...
  switch (m_state)
  {
	...
	//Inserção: Carlos --> Agendando inserção da interferência no radar.
	Simulator::Schedule (delay, &LteSpectrumPhy::AddSUInterference, this, UL_SRS_DURATION, delay);
 
      	//m_radar -> SetTxDuration (UL_SRS_DURATION);

      	//m_radar->SetPositionTx (m_mobility->GetPosition()); 
      	//Time t_now = Simulator::Now ();
      	//m_radar->CalculateSUInterference (t_now);
	Simulator::Schedule (UL_SRS_DURATION, &LteSpectrumPhy::EndRadarInterf, this);
...
//  	m_radar->EndInterference ();
//  }

}

//Inserção: Carlos --> Retirando interferencia do radar.
void
LteSpectrumPhy::EndRadarInterf () {
  ...
  if (lteDataRxParams != 0)
    {
      ...
      //Inserção: Carlos --> Pegar informacao de posicao do no
      m_radar->SetPositionRx (m_mobility->GetPosition());
  ...
  else
    {
	  ...
	  if (lteDlCtrlRxParams != 0) {
	  	...
	  	//std::cout << "CellId para Controle: " << lteDlCtrlRxParams->cellId << std::endl;
	  } else {

		//std::cout << "CellId para Controle: " << lteUlSrsRxParams->cellId << std::endl;
	  }
      ...
      else
        {
         ...
         //Inserção: Carlos --> Pegar informacao de posicao do no
          m_radar->SetPositionRx (m_mobility->GetPosition());
	  //std::cout << "Interferência" << std::endl;
...
LteSpectrumPhy::EndRxData ()
{
  ...
  //Inserção: Carlos --> Verificando se houve interrupção na recepção por troca de canal.

  if (m_interferenceData->GetLoss ()) {
	//std::cout << "Transmission interrupted. Loss Transport Block." << std::endl;
	m_sinrPerceived -= m_sinrPerceived;
  }

  m_sinrPerceived *= m_txModeGain.at (m_transmissionMode);

  //std::cout << "SINR Perceived: " << m_sinrPerceived << std::endl;
...
//Inserção: Carlos --> Adicionando método Get para o objeto radar.

Ptr<radar>
LteSpectrumPhy::GetRadar () {

 return m_radar;

}

//Inserção: Carlos --> Método para setar objeto AggregateInterference em m_radar

void
LteSpectrumPhy::SetAggregateInterference (Ptr<AggregateInterference> agg) {
	m_radar->SetAggInt (agg);
	m_agg = agg;
}

Ptr<AggregateInterference>
LteSpectrumPhy::GetAgg () {

	return m_agg;
}

void
LteSpectrumPhy::SetPropagationDelay (Ptr<PropagationDelayModel> prop) {

	m_propagationDelay = prop;
}
